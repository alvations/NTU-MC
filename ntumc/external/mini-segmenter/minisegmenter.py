...
def readCedict(dic, output=None):
...
def getNodes (chunk,dic):
  if dic == None:
    dic = sorted(readCedict('cedict_ts.u8'),key=len)
...
def segmenter(sentence,dic=sorted(readCedict('cedict_ts.u8'),key=len)):
  #print dic
  #print sentence
  chunked_sent = []
...
def minidemo():
	...
	cmn_dic = sorted(readCedict('cedict_ts.u8'),key=len)
